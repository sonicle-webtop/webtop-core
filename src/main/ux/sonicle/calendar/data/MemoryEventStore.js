 ...
 */
Ext.define('Sonicle.calendar.data.MemoryEventStore', {
    ...
    constructor: function(config){
        ...
        this.sorters = this.sorters || [{
            property: Sonicle.calendar.data.EventMappings.StartDate.name,
            direction: 'ASC'
        }];
        
        this.idProperty = this.idProperty || Sonicle.calendar.data.EventMappings.EventId.mapping || 'id';
        this.fields = Sonicle.calendar.data.EventModel.getFields();
    ...
    interceptCreateRecords: function(records, operation, success) {
        if (success) {
            ...
            for (; i < len; i++) {
                records[i].data[Sonicle.calendar.data.EventMappings.EventId.name] = records[i].id;
            }
        }
    },
