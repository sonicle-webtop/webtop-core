/**
 * @class Sonicle.calendar.form.EventDetails
 ...
 * calendar components. It is also configured to automatically bind records of type {@link Sonicle.calendar.EventRecord}
 ...
 */
Ext.define('Sonicle.calendar.form.EventDetails', {
    extend: 'Ext.form.Panel',
    alias: 'widget.eventeditform',
    
    requires: [
        'Sonicle.calendar.form.field.DateRange',
        'Sonicle.calendar.form.field.ReminderCombo',
        'Sonicle.calendar.data.EventMappings',
        'Sonicle.calendar.form.field.CalendarCombo'
    ],
     ...
     * @param {Sonicle.calendar.form.EventDetails} this
     ...
     * @param {Sonicle.calendar.form.EventDetails} this
     * @param {Sonicle.calendar.EventRecord} rec The new {@link Sonicle.calendar.EventRecord record} that was deleted
     */
    ...
    initComponent: function() {
        this.titleField = new Ext.form.Text({
            fieldLabel: 'Title',
            name: Sonicle.calendar.data.EventMappings.Title.name,
            emptyText: 'Event Title',
            allowBlank: false,
            anchor: '90%'
        });
        this.dateRangeField = new Sonicle.calendar.form.field.DateRange({
            fieldLabel: 'When',
            singleLine: false,
            anchor: '90%'
        });
        this.reminderField = new Sonicle.calendar.form.field.ReminderCombo({
            name: 'Reminder',
            anchor: '70%'
        });
        this.notesField = new Ext.form.TextArea({
            fieldLabel: 'Notes',
            name: Sonicle.calendar.data.EventMappings.Notes.name,
            grow: true,
            growMax: 150,
            anchor: '100%'
        });
        this.locationField = new Ext.form.Text({
            fieldLabel: 'Location',
            name: Sonicle.calendar.data.EventMappings.Location.name,
            anchor: '100%'
        });
        this.urlField = new Ext.form.Text({
            fieldLabel: 'Web Link',
            name: Sonicle.calendar.data.EventMappings.Url.name,
            anchor: '100%'
        });
        ...
        if (this.calendarStore) {
            this.calendarField = new Sonicle.calendar.form.field.CalendarCombo({
                store: this.calendarStore,
                anchor: '70%',
                name: Sonicle.calendar.data.EventMappings.CalendarId.name
            });
    ...
    loadRecord: function(rec){
        ...
        if(this.calendarStore){
            this.form.setValues({
                'calendar': rec.data[Sonicle.calendar.data.EventMappings.CalendarId.name]
            });
        }
