...
public class ManagerUtils {
	static Integer toOffset(Integer page, Integer limit) {
		return limit * (page-1);
	}
	...
	static DomainAccessLogDetail createDomainAccessLogDetail(ODomainAccessLogDetail src) {
		if (src == null) return null;
		return fillDomainAccessLogDetail(new DomainAccessLogDetail(), src);
	}
	
	static <T extends DomainAccessLogDetail> T fillDomainAccessLogDetail(T tgt, ODomainAccessLogDetail src) {
		if ((tgt != null) && (src != null)) {
			tgt.setTimestamp(src.getTimestamp());
			tgt.setAction(src.getAction());
