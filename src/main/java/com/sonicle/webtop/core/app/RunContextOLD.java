 ...
 * @author malbinola
 */
public class RunContextOLD {
	private final ServiceContext serviceContext;
	private final Subject subject;
	private final UserProfile.Id subjectProfile;
	
	RunContextOLD(ServiceContext serviceContext, Subject subject) {
		this.serviceContext = Check.notNull(serviceContext);
		this.subject = Check.notNull(subject);
		this.subjectProfile = Check.notNull(RunContext.getProfileId(subject));
	}
	
	public RunContextOLD(ServiceContext serviceContext) {
		this.serviceContext = Check.notNull(serviceContext);
		this.subject = Check.notNull(RunContext.getSubject());
		this.subjectProfile = Check.notNull(RunContext.getProfileId(subject));
	}
	...
	public boolean isPermitted(String serviceId, String resource) {
		ensureSameSubject();
		return RunContext.isPermitted(subjectProfile, serviceId, resource);
	}
	
	public boolean isPermitted(String serviceId, String resource, String action) {
		ensureSameSubject();
		return RunContext.isPermitted(subjectProfile, serviceId, resource, action);
	}
	
	public boolean isPermitted(String serviceId, String resource, String action, String instance) {
		ensureSameSubject();
		return RunContext.isPermitted(subjectProfile, serviceId, resource, action, instance);
	}
	
	public boolean isSysAdmin() {
		ensureSameSubject();
		return RunContext.isSysAdmin(subjectProfile);
	}
	
	public boolean isWebTopAdmin() {
		ensureSameSubject();
		return RunContext.isWebTopAdmin(subjectProfile);
	}
	
	public void ensureIsPermitted(String serviceId, String resource) throws AuthException {
		ensureSameSubject();
		RunContext.ensureIsPermitted(subjectProfile, serviceId, resource);
	}
	
	public void ensureIsPermitted(String serviceId, String resource, String action) throws AuthException {
		ensureSameSubject();
		RunContext.ensureIsPermitted(subjectProfile, serviceId, resource, action);
	}
	
	public void ensureIsPermitted(String serviceId, String resource, String action, String instance) throws AuthException {
		ensureSameSubject();
		RunContext.ensureIsPermitted(subjectProfile, serviceId, resource, action, instance);
	}
	
	public void ensureIsSysAdmin() throws AuthException {
		ensureSameSubject();
		RunContext.ensureIsSysAdmin(subjectProfile);
	}
	
	public void ensureIsWebTopAdmin() throws AuthException {
		ensureSameSubject();
		RunContext.ensureIsWebTopAdmin(subjectProfile);
	}
