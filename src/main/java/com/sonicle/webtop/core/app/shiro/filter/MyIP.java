 ...
 * @author malbinola
 */
public class MyIP extends PathMatchingFilter {
	...
	protected boolean onPreHandle(ServletRequest request, ServletResponse response, Object mappedValue) throws Exception {
		...
		try {
			if (ServletUtils.isGet(httpRequest)) {
				WebTopApp wta = getWebTopApp(httpRequest);
				int raw = ServletUtils.getIntParameter(request, "raw", 0);
				if (raw == 1) {
					writePlain(ServletUtils.getClientIP(httpRequest), httpResponse);
					...
					writePage(wta, locale, ServletUtils.getClientIP(httpRequest), httpResponse);
				}

			} else {
				ServletUtils.sendError(httpResponse, 405); // Method Not Allowed
			}
			
		} catch (Exception ex) {
			LOGGER.trace("[{}] Error onPreHandle", ServletUtils.getRequestID(httpRequest), ex);
		} finally {
			return false;
		}
	}
