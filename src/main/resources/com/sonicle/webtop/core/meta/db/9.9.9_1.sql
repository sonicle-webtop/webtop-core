...
CREATE VIEW "core"."vw_access_log" AS SELECT
...
FROM
	crosstab ( 'select ARRAY[session_id, domain_id, user_id]::text[] as row_details, session_id, domain_id, user_id, "data", "action", "timestamp" from core.audit_log where 
		service_id=''com.sonicle.webtop.core'' and context=''AUTH'' order by session_id, user_id, action, "timestamp"' :: TEXT, 'select * from (values (''LOGIN_SUCCESS''),(''OTP_SUCCESS''),(''OTP_FAILURE''),(''OTP_ABANDONED''),(''AUTHENTICATED''),(''LOGOUT''),(''SESSION_EXPIRED''),(''LOGIN_FAILURE'')) as actions' :: TEXT ) crosstab (
...
CREATE VIEW "core"."vw_auth_details" AS SELECT
	...
	date_part(
		'minute' :: TEXT,
	( COALESCE ( vw_access_log."LOGOUT", vw_access_log."SESSION_EXPIRED" ) - vw_access_log."LOGIN_SUCCESS" )) AS minutes,
CASE
		
		WHEN ( vw_access_log."AUTHENTICATED" IS NULL ) THEN
	FALSE ELSE TRUE 
...
CASE
		...
		WHEN ((
			...
			AND ( vw_access_log."OTP_FAILURE" IS NULL )) THEN
		FALSE ELSE TRUE 
		END AS failure,
		((
			SELECT COUNT
