 ...
 */
Ext.define('Sonicle.webtop.core.ux.CopyBox', {
			...
			items: [
				{
					...
					triggers: {
						copy: WTF.copyTrigger({
							tooltip: Sonicle.Utils.stringCoalesce(me.copyText, WT.res('copyBox.copyText'))
						})
					},
					/*
	...
	setCustomPromptValue: function(value) {
		var cp = this.customPrompt;
		cp.lookupReference('value').setValue(value);
	},
	
	getCustomPromptValue: function() {
		var cp = this.customPrompt;
		return cp.lookupReference('value').getValue();
	}
});
