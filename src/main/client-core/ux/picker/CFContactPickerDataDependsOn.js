 ...
 */
Ext.define('Sonicle.webtop.core.ux.picker.CFContactPickerDataDependsOn', {
	alternateClassName: 'WTA.ux.picker.CFContactPickerDataDependsOn',
		...
		me.add({
			...
			items: [
				WTF.localCombo(me.fieldsStoreValueField, me.fieldsStoreDisplayField, {
					...
					allowBlank: false,
					fieldLabel: WT.res('customFieldContactPickerDataDependsOn.parentField.lbl'),
					anchor: '100%'
				}),
				WTF.localCombo('field1', 'field2', {
					reference: 'keyword',
					bind: '{data.keyword}',
					store:{
						type: 'array',
						...
						data: [
							['companyId', WT.res('customFieldContactPickerDataDependsOn.keyword.companyId')]
						]
					},
					allowBlank: false,
					fieldLabel: WT.res('customFieldContactPickerDataDependsOn.keyword.lbl'),
					anchor: '100%'
				})
			]
		});
	...
	updateValue: function(nv, ov) {
		...
		Sonicle.VMUtils.setData(this.getViewModel(), data, ['parentField', 'keyword']);
	},
	
	syncValue: function() {
		var data = Sonicle.VMUtils.getData(this.getViewModel(), ['parentField', 'keyword']);
	...
	privates: {
		doValidate: function() {
			...
			Ext.iterate(['parentField', 'keyword'], function(name) {
	...
	statics: {
		format: function(data, fieldsStore) {
			...
			return Sonicle.String.join(' ', parentField + '.value', '->', data.keyword);
		}
	}
});
